...
describe("Orgs (e2e)", () => {
  ...
  it("creates an org with 4 system roles and an owner membership", async () => {
    ...
    const res = await agent().post("/api/orgs").set("Cookie", cookies).send({ name: "Acme" })
  ...
  it("lists only the caller's orgs", async () => {
    ...
    await agent().post("/api/orgs").set("Cookie", a.cookies).send({ name: "A-org" })
    const list = await agent().get("/api/orgs").set("Cookie", b.cookies)
  ...
  it("reads, updates, and deletes an org by permission", async () => {
    ...
    const created = await agent().post("/api/orgs").set("Cookie", cookies).send({ name: "Acme" })
    const id = created.body.data.id
    expect((await agent().get(`/api/orgs/${id}`).set("Cookie", cookies)).status).toBe(200)
    const upd = await agent().put(`/api/orgs/${id}`).set("Cookie", cookies).send({ name: "Acme 2" })
    expect(upd.body.data.name).toBe("Acme 2")
    expect((await agent().delete(`/api/orgs/${id}`).set("Cookie", cookies)).status).toBe(200)
  })

  it("returns snake_case fields on org reads", async () => {
    const { cookies } = await signupAndSignin(app)
    const created = await agent().post("/api/orgs").set("Cookie", cookies).send({ name: "Acme" })
    const res = await agent().get(`/api/orgs/${created.body.data.id}`).set("Cookie", cookies)
  ...
  it("rejects a malformed org id with 400", async () => {
    const { cookies } = await signupAndSignin(app)
    const res = await agent().get("/api/orgs/not-a-uuid").set("Cookie", cookies)
  ...
  it("returns 404 for a missing org and 403 for a non-member", async () => {
    ...
    const created = await agent()
      ...
      .send({ name: "Acme" })
    const id = created.body.data.id
    const notMember = await agent().get(`/api/orgs/${id}`).set("Cookie", outsider.cookies)
